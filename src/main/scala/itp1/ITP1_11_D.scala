...
object ITP1_11_D extends App {
  ...
  if (isAllDifferent(lines)) {
    println("Yes")
  } else {
    println("No")
  }

  def isAllDifferent(lines: Array[String]): Boolean = {
    val n = lines.length
    for (i <- 0 until n; j <- 0 until n) {
      if (i != j) {
        val dice1: Dice = new Dice(lines(i).split(" "))
        val dice2: Dice = new Dice(lines(j).split(" "))
        if (dice1.isIdentical(dice2)) {
          return false
        }
      }
    }
